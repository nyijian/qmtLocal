...
本仓库所有 `.py` / `.md` 源文件统一用 **GBK**（无BOM）保存，因为策略最终要在QMT客户端
的策略编辑器里运行，而QMT编辑器固定按GBK读写；UTF-8的脚本拷进去会直接乱码。

- 每个 `.py` 首行都是 `# coding:gbk`（Python 3 读非UTF-8源码必须有这行，否则直接 `SyntaxError`）。
- VSCode 里若看到乱码，用 `Reopen with Encoding` 选 `GBK`（可在 `.vscode/settings.json` 里
  把 `files.encoding` 设为 `gbk` 免得每次手动切）。
- `qmt_mock.runner` 读策略源码时会先按UTF-8严格解、失败再回退GBK，所以两种编码的脚本都能跡。
