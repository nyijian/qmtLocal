...
本仓库的 `.py` 按**运行位置**分成两类，编码不同，不要混：

| 文件 | 编码 | 原因 |
|---|---|---|
| 要粘进QMT策略编辑器的（`main.py`、`qmt_client_scripts/*`） | **GBK**（无BOM），首行 `# coding:gbk` | QMT编辑器固定按GBK读写，UTF-8的脚本拷进去直接乱码 |
| 只在本地跑的（`tick_subscribe.py`、`qmt_bridge/*`） | **UTF-8（带BOM）** | 见下面这条坑 |
| `.md` | GBK | 跟仓库其余部分一致 |

**坑：GBK源码没法用 `python xxx.py` 直接跑。** 本机的 Python 3.6.8（venv 里的那个和
`D:\国金证券QMT交易端\bin.x64\python.exe` 表现完全一样）把GBK文件当**顶层脚本**解析时，
只要文件里含中文的行超过10行，就报：
...
同一个文件被 `import`（包括 `python -m 包.模块`）完全正常，`py_compile` 也正常，
只有 `python 文件.py` 这一条路会炸。实测阈值是行数不是字节数：10行中文通过，11行开始报错，
而单行16KB的GBK文件不炸。

所以：**凡是要直接 `python xxx.py` 跑的文件一律用 UTF-8。** 选「带BOM」是为了让
`.vscode/settings.json` 里的 `files.encoding: gbk` 不影响它们——VSCode 看到BOM会自动按
UTF-8打开，既不用改全局设置，也不用打开 `autoGuessEncoding`。

- `qmt_mock/*` 现在还是GBK。它只通过 `python -m qmt_mock.runner` 跑（走import路径），
  所以没踩到这个坑；哪天想直接 `python qmt_mock/runner.py`，先转成UTF-8。
- VSCode 里若看到GBK文件乱码，用 `Reopen with Encoding` 选 `GBK`（`files.encoding` 已设为 `gbk`，
  正常情况下不用手动切）。
- `qmt_mock.runner` 读策略源码时会先按UTF-8严格解、失败再回退GBK，所以两种编码的策略脚本都能跑。
  顺序不能反：UTF-8的中文字节按GBK解不报错，只会静默变成乱码。
- 注意 `git diff` / GitHub 网页会把GBK中文显示成乱码，这是展示问题，文件本身没坏。
